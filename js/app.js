 ...
 */
    ...
    (() => {
        ...
        let formValidate = {
            ...
            validateInput(formRequiredItem) {
                ...
                if (formRequiredItem.dataset.required === "email") {
                    ...
                    formRequiredItem.value = formRequiredItem.value.replace(" ", "").replace("-", "");
                    let valExp3 = new RegExp("_");
                    if (formRequiredItem.value.length < 17 || valExp3.test(formRequiredItem.value)) {
                        this.addError(formRequiredItem);
                        error++;
                    } else this.removeError(formRequiredItem);
        ...
        class SelectConstructor {
            ...
            getSelectTitleValue(selectItem, originalSelect) {
                ...
                if (originalSelect.hasAttribute("data-search")) return `<div class="${this.selectClasses.classSelectTitle}"><span${pseudoAttribute} class="${this.selectClasses.classSelectValue}"><input autocomplete="off" type="text" placeholder="${selectTitleValue}" data-placeholder="${selectTitleValue}" class="${this.selectClasses.classSelectInput}"></span></div>`; else {
                    ...
                    if (originalSelect.dataset.ariaLabel) return `<button type="button" class="${this.selectClasses.classSelectTitle}" aria-label="${originalSelect.dataset.ariaLabel}" ><span${pseudoAttribute} class="${this.selectClasses.classSelectValue}${pseudoAttributeClass}"><span class="${this.selectClasses.classSelectContent}${customClass}">${selectTitleValue}</span></span></button>`; else return `<button type="button" class="${this.selectClasses.classSelectTitle}"><span${pseudoAttribute} class="${this.selectClasses.classSelectValue}${pseudoAttributeClass}"><span class="${this.selectClasses.classSelectContent}${customClass}">${selectTitleValue}</span></span></button>`;
                }
            }
        ...
        toggleSearchBtn.addEventListener("click", (function(event) {
            ...
            if (searchContainer.style.display === "none" || searchContainer.style.display === "") searchContainer.style.display = "block"; else searchContainer.style.display = "none";
        }));
        document.addEventListener("click", (function(event) {
            if (!searchForm.contains(event.target)) searchContainer.style.display = "none";
        }));
        if (document.querySelector("[data-fileinput]")) {
            const popupInputFiles = document.querySelectorAll("[data-fileinput]");
            popupInputFiles.forEach((popupInputFile => {
                const formImage = popupInputFile.querySelector("[data-fileinput-input]");
                const formPreview = popupInputFile.querySelector("[data-fileinput-button] span");
        ...
        const app_isMobile = {
            ...
            any: function() {
                return app_isMobile.Android() || app_isMobile.BlackBerry() || app_isMobile.iOS() || app_isMobile.Opera() || app_isMobile.Windows();
            }
        };
        if (app_isMobile.any()) {
